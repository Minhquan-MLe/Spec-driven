...
exports.app.get('/dashboard', (c) => {
    const ailmentRows = (0, store_1.listAilments)()
        .map((a) => `<tr><td>${a.id}</td><td>${a.category}</td><td>${a.title}</td><td><mark>${a.status}</mark></td></tr>`)
        .join('');
    const therapyRows = (0, store_1.listTherapies)()
        .map((t) => `<tr><td>${t.name}</td><td>${t.categories.join(', ')}</td></tr>`)
        .join('');
    const appointmentRows = (0, store_1.listAppointments)()
        ...
        return `<tr><td>${appt.agentId}</td><td>${(_a = therapy === null || therapy === void 0 ? void 0 : therapy.name) !== null && _a !== void 0 ? _a : 'Unknown'}</td><td>${(_b = slot === null || slot === void 0 ? void 0 : slot.timeSlot) !== null && _b !== void 0 ? _b : 'Unknown'}</td></tr>`;
    })
        .join('');
    const content = `
    <h1>Dashboard</h1>
